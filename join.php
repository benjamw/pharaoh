...
// $Id: join.php 210 2009-10-24 11:02:01Z cchristensen $
...
if ('none' != $extra_info['conquer_type']) {
	...
	// set the default minimum to 0
	if (empty($minimum) || (0 > $minimum) || ! (int) $minimum) {
		$minimum = 0;
	}
	...
	$conquests = array( );
	for ($amount = 1; $amount < 200; ++$amount) {
		$limit = max((((((int) floor($amount / $per_number)) + 1) - $skip) * $conquests_per), 0) + $start_at;
	...
	$equation = "max( ( ( ( floor( x / {$per_number} ) + 1 ) - {$skip} ) * {$conquests_per} ) , 0 ) + {$start_at}";
...
EOT;
	...
	$invite_players = array( );
	foreach ($players_full as $player) {
		$invite_players[] = $player['player_id'];
	}
	...
	$players = array_diff($invite_players, array_merge($players_joined, $players_maxed));
...
if (is_array($game_players) && count($game_players)) {
	...
	foreach ($game_players as $player_id => $player) {
		...
		$contents .= '
					<li class="'.substr($player['color'], 0, 3).'">'.( ! empty($player['host']) ? '&ndash; ' : '').$player['object']->username.'</li>';
	}
